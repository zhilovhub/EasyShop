...
async def _recreate_blob_file(file_path: str, extension: str | None = None) -> str:
    ...
    if extension.lower() not in ACCEPTED_PHOTO_EXTENSIONS:
        raise HTTPUnacceptedError(
            file_name=old_file_name, provided_extension=extension, accepted_extensions=str(ACCEPTED_PHOTO_EXTENSIONS)
        )

    new_file_name = f"{old_file_name}{extension}"
    api_logger.info(
        f"renaming .blob file to new filename with detected extension: {extension} "
    ...
    try:
        ...
        api_logger.debug(f"new file created ({new_file_name}) " f"returning in api method new file except of old .blob")
...
async def get_file(file_name: str) -> FileResponse:
    ...
    try:
        ...
        api_logger.error("Error while execute get_file", exc_info=e)
...
async def get_product_thumbnail(product_id: int) -> FileResponse:
    ...
    headers = {"accept-ranges": "bytes", "strict-transport-security": "max-age=63072000"}
    ...
    try:
        ...
        api_logger.debug(f"product_id={product_id}: there is not thumbnail with {thumbnail_path}, creating new")
        try:

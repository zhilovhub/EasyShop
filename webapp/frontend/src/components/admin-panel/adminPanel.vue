  ...
  <div v-else>
    <div class="wrapper">
      ...
      <div class="images">
        ...
        <svg @click="toggleAddProduct" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <input v-model="inputValue" placeholder="Введите название или ID">
        ...
        <ul v-else class="items-styles">
          <li style="list-style-type: none" v-for="item in filteredItems">
            <Transition>
          ...
          <div class="block-footer" v-show="item.isActive">
            ...
            <div>
              ...
              <span>{{item.category ? item.category[0] : '' }}</span>
            </div>
            <hr>
            <div>
              <span>Цена</span>
        ...
        <Transition>
        <div v-show="deleteModelWindowIsActive" class="model-wrapper">
...
export default {
  data() {
    return {
      ...
      addProductComponentIsActive: false,
  ...
  computed: {
    ...
    filteredItems() {
      ...
      return this.items.filter(el => {
        return !this.inputValue || (el.name && el.name.toLowerCase().includes(this.inputValue.toLowerCase()));
      });
    },
  },
  methods: {
    ...
    toggleSelected(item) {
      ...
      this.mainCircleIsActive = this.items.length === this.itemsForDelete.length;
    },
    ...
    deleteProducts() {
      const deletePromises = this.itemsForDelete.map(item => {
        return this.$store.dispatch('deleteProduct', item.id);
      });
      this.closeModelWindow();
      this.isLoading = true;
      // Ждем выполнения всех промисов с помощью Promise.all

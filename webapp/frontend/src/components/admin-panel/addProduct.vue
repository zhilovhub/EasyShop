...
<div class="wrapper" :style="{ opacity: isMounted ? 1 : 0 }">
  ...
  <div class="main">
    <div class="card">
      <h1>Категория</h1>
      <div @click="toggleFooter($event)" class="category-block" :style="{ borderRadius: selectingCategoryIsActive ? '15px 15px 0 0' : '15px'}">
    ...
    <div class="card">
      ...
      <input v-model="productName" placeholder="Введите название">
    </div>
    <div class="card">
      <h1>Описание</h1>
      <input v-model="productDescription" placeholder="Напишите описание">
    </div>
    <div class="card">
      <h1>Артикул</h1>
      <input v-model="productArticle" placeholder="Введите артикул">
    </div>

...
async def create_order(user_id: int, data, order_type: OrderType, _json: bool = True) -> OrderSchema:
    ...
    current_logger.info(
        f"user_id={user_id}: received web app data: {data}", extra=extra_params(user_id=user_id, bot_id=bot_id)
    )
        ...
        if "chosen_options" in item and item["chosen_options"]:
            for option in item["chosen_options"]:
                ...
                chosen_options.append(
                    OrderItemExtraOption(name=option["name"], selected_variant=option["selected_variant"], price=price)
                )

        items[product_id] = OrderItem(amount=item["amount"], used_extra_options=chosen_options)
...
async def _handle_zero_products(event: Message, bot_owner: int, zero_products: list, order_type: OrderType) -> None:
    if zero_products:
        ...
        match order_type:
            ...
            case OrderType.CUSTOM_BOT_ORDER:
                msg = await Bot(main_telegram_bot_settings.TELEGRAM_TOKEN, default=BOT_PROPERTIES).send_message(
                    bot_owner, text
                )
            case _:
                raise UnknownOrderType(order_type)

...
class Scheduler:
    ...
    async def change_job(self, job, changes):
        logger.debug(
            f"job_id={job.id}: job changed by {changes}",
            extra=extra_params(job_id=job.id)
        )
    ...
    async def get_jobs(self):
        ...
        logger.debug(
            f"Scheduler returned {len(jobs)} jobs: {jobs}"
        )
        return jobs
    ...
    async def get_job(self, job_id: str):
        ...
        if job:
            logger.debug(
                f"job_id={job_id}: found {job}",
                extra=extra_params(job_id=job_id)
            )
        else:
            logger.warning(
                f"job_id={job_id}: this job is not found",
                extra=extra_params(job_id=job_id)
            )
    ...
    async def del_job(self, job):
        logger.debug(
            f"job_id={job.id}: the job has been deleted",
            extra=extra_params(job_id=job.id)
        )
    ...
    async def del_job_by_id(self, job_id: str):
        logger.debug(
            f"job_id={job_id}: the job has been deleted",
            extra=extra_params(job_id=job_id)
        )
    ...
    async def generate_job_id(self, length: int = 10) -> str:
        ...
        if not await self.get_job(new_id):
            logger.debug(
                f"job_id={new_id}: the job_id={new_id} has been generated",
                extra=extra_params(job_id=new_id)
            )
            return new_id

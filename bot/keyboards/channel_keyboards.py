...
class ReplyBackChannelMenuKeyboard:
    ...
    def get_keyboard() -> ReplyKeyboardMarkup:
        ...
        return ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text=actions.BACK_TO_CHANNEL_MENU.value)]], resize_keyboard=True
        )
...
class InlineChannelPublishAcceptKeyboard:
    ...
    @staticmethod
    @callback_json_validator
    def callback_json(action: Callback.ActionEnum, bot_id: int, msg_id, product_id: int, channel_id: int | None) -> str:
        return InlineChannelPublishAcceptKeyboard.Callback(
            a=action, bot_id=bot_id, chid=channel_id, pid=product_id, mid=msg_id
    ...
    @staticmethod
    def get_keyboard(bot_id: int, msg_id: int, product_id: int, channel_id: int) -> InlineKeyboardMarkup:
...
class InlineChannelsListPublishKeyboard:
    ...
    def callback_json(
        ...
        return InlineChannelsListPublishKeyboard.Callback(
            a=action, bot_id=bot_id, chid=channel_id, mid=msg_id, pid=product_id
        ).model_dump_json(by_alias=True)
    ...
    @staticmethod
    async def get_keyboard(bot_id: int, msg_id: int, product_id: int) -> InlineKeyboardMarkup:
...
class InlineChannelsListKeyboard:
    ...
    @staticmethod
    @callback_json_validator
    def callback_json(action: Callback.ActionEnum, bot_id: int, channel_id: int | None) -> str:
        return InlineChannelsListKeyboard.Callback(a=action, bot_id=bot_id, channel_id=channel_id).model_dump_json(
            by_alias=True
        )
    ...
    async def get_keyboard(bot_id: int) -> InlineKeyboardMarkup:
        ...
        resized_channels_buttons = await _get_resized_channels_button(
            keyboard=InlineChannelsListKeyboard, action=actions.OPEN_CHANNEL, bot_id=bot_id
        )
            ...
            inline_keyboard=[
                *resized_channels_buttons,
                [
                    InlineKeyboardButton(
                        text="🔙 Назад",
                        callback_data=InlineChannelsListKeyboard.callback_json(actions.BACK_TO_MAIN_MENU, bot_id, None),
                    ),
                    InlineKeyboardButton(
...
class InlineChannelMenuKeyboard:
    ...
    @callback_json_validator
    def callback_json(
        action: Callback.ActionEnum, bot_id: int, channel_id: int, post_message_id: int | None = None
    ) -> str:
        to_exclude = set()
        ...
        return InlineChannelMenuKeyboard.Callback(
            a=action, bot_id=bot_id, channel_id=channel_id, post_message_id=post_message_id
    ...
    @staticmethod
    async def get_keyboard(bot_id: int, channel_id: int) -> InlineKeyboardMarkup:
        ...
        if channel_post:
            ...
            channel_post_button = InlineKeyboardButton(
                text="Создать запись",
                callback_data=InlineChannelMenuKeyboard.callback_json(actions.CREATE_POST_MESSAGE, bot_id, channel_id),
            )
        ...
        if channel_contest and post_message:
            channel_contest_button = InlineKeyboardButton(
                ...
                callback_data=InlineChannelMenuKeyboard.callback_json(actions.EDIT_CONTEST, bot_id, channel_id),
            )
        else:
            channel_contest_button = InlineKeyboardButton(
                text="Создать конкурс",
                callback_data=InlineChannelMenuKeyboard.callback_json(actions.CREATE_CONTEST, bot_id, channel_id),
            )
        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    ...
                    InlineKeyboardButton(
                        text="Аналитика",
                        callback_data=InlineChannelMenuKeyboard.callback_json(actions.ANALYTICS, bot_id, channel_id),
                    ),
                ],
                [channel_contest_button],
                [
                    InlineKeyboardButton(
                        text="Права бота", url=f"https://t.me/{await get_bot_username(bot_id)}?startchannel"
                    )
                ],
                [
                    InlineKeyboardButton(
...
class InlineContestTypeKeyboard:
    ...
    @staticmethod
    @callback_json_validator
    def callback_json(action: Callback.ActionEnum, bot_id: int, channel_id: int) -> str:
        return InlineContestTypeKeyboard.Callback(a=action, bot_id=bot_id, channel_id=channel_id).model_dump_json(
            by_alias=True
        )
        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🎲 Рандомайзер",
                        callback_data=InlineContestTypeKeyboard.callback_json(actions.RANDOMIZER, bot_id, channel_id),
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def _get_resized_channels_button(
    ...
    if keyboard == InlineChannelsListKeyboard:
        channels_buttons = [
            InlineKeyboardButton(
                text="@" + channel[1],
                callback_data=keyboard.callback_json(action, bot_id=bot_id, channel_id=channel[0].channel_id),
            )

...
async def start_custom_bot(token: str):
    async with (aiohttp.ClientSession() as session):
        async with session.get(
                f"{config.LOCAL_API_SERVER_HOST}:{config.LOCAL_API_SERVER_PORT}/start_bot/{token}") as response:
...
async def stop_custom_bot(token: str):
    async with (aiohttp.ClientSession() as session):
        async with session.get(
                f"{config.LOCAL_API_SERVER_HOST}:{config.LOCAL_API_SERVER_PORT}/stop_bot/{token}") as response:
            if response.status != 200:
                raise LocalAPIException(f"API returned {response.status} status code "
                                        f"with text {await response.text()}")

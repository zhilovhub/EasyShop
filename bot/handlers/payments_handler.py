...
class PreCheckHandler(PreCheckoutQueryHandler):
    async def handle(self) -> Any:
        try:
            ...
            logger.info(f"new main bot payment pre checkout with payload : payload={payload} "
            ...
            logger.error(f"unhandled error while processing payment for user {self.event.from_user.id}",
                         exc_info=True, extra=extra_params(user_id=self.event.from_user.id))
...
async def process_successfully_payment(message: Message):
    ...
    if "TEST" in payload:
        ...
        pay_id = "TEST"
        await message.answer("Введенные данные при тестовой оплате:"
